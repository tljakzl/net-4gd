...
namespace n4gd {
    Connection::Connection(Ipv4::ADDRESS protocol, float timeout)
            : _protocol(protocol), _timeout(timeout) {
    }
    ...
    bool Connection::SendPacket(const unsigned char *data, int size) {
        ...
        return _socket.Send(_address, reinterpret_cast<const void *>(packet.data()), size + 4);
    }
    ...
    int Connection::ReceivePacket(unsigned char *data, int size) {
        ...
        packet.resize(size + 4);
        Ipv4 sender;
        int bytesRead = _socket.Receive(sender, reinterpret_cast<void *>(packet.data()), size + 4);
